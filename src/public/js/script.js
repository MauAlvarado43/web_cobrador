...
const validateReport = () => {
    ...
    let sucursal = document.getElementById("sucursalReport")
    let sucursalHelp = document.getElementById("sucursalReportHelp")
    let lapse = document.getElementById("lapseReport")
    let lapseHelp = document.getElementById("lapseReportHelp")

    lapse.classList.remove("border-danger")
    sucursal.classList.remove("border-danger")
    sucursalHelp.innerText = ""
    lapseHelp.innerText = ""

    if(sucursal.value == "0"){
        sucursal.classList.add("border-danger")
        sucursalHelp.innerText = "Seleccione una sucursal válida"
        return
    }

    if(lapse.value != "7" && lapse.value != "15" && lapse.value != "31" && lapse.value != "62"){
        lapse.classList.add("border-danger")
        lapseHelp.innerText = "Seleccione un plazo válido"
        return
    }

    let confirmationReport = new bootstrap.Modal(document.getElementById('confirmationReport'), {
        keyboard: false
    })

    confirmationReport.show()
...
const generateReport = async (e, save) => {
    ...
    let response = await fetch('/generateReportLendings', {
        ...
        body: JSON.stringify(
            {
                save: save,
                lapse: encrypt(lapse.value),
                name: encrypt(sucursal.value)
            }
        )
    })
...
const getState = async (e) => {
    ...
    let sucursal = document.getElementById("sucursalState")

    let lapseHelp = document.getElementById("lapseStateHelp")
    let sucursalHelp = document.getElementById("sucursalStateHelp")

    sucursal.classList.remove("border-danger")
    lapse.classList.remove("border-danger")
    lapseHelp.innerHTML = ""
    sucursalHelp.innerHTML = ""

    if(lapse.value != "7" && lapse.value != "15" && lapse.value != "31" && lapse.value != "62"){
        lapse.classList.add("border-danger")
        lapseHelp.innerHTML = "Seleccione un plazo válido"
        return
    }
    if(sucursal.value == "0" || sucursal.value == ""){
        sucursal.classList.add("border-danger")
        sucursalHelp.innerHTML = "Seleccione una sucursal válida"
        return
    }
...
const registerEmployee = async (e) => {
    ...
    let suc = document.getElementById("sucEmp")
    ...
    let sucHelp = document.getElementById("sucEmpHelp")
    ...
    suc.classList.remove("border-danger")
    ...
    if(suc.value == "0"){
        suc.classList.add("border-danger")
        sucHelp.innerHTML = "Seleccione una sucursal válida"
        return
    }
...
const registerSucursal = async(e) => {
    
    e.preventDefault()

    let name = document.getElementById("nameSuc")
    let dom = document.getElementById("zonaSuc")

    let nameHelp = document.getElementById("nameSucHelp")
    let domHelp = document.getElementById("zonaSucHelp")
    ...
    if(name.value == ""){
        name.classList.add("border-danger")
        nameHelp.innerHTML = "El nombre de la sucursal no puede estar vacío"
        return
    }
    if(dom.value == ""){
        dom.classList.add("border-danger")
        domHelp.innerHTML = "El domicilio de la sucursal no puede estar vacío"
        return
    }

    document.body.style.cursor = 'progress'

    let response = await fetch('/registerSucursal', {
        method: 'POST',
        mode: 'cors',
        cache: 'no-cache',
        credentials: 'same-origin',
    ...
    if(json.code == 201){
        alert.innerHTML = getAlert("success", "Nueva sucursal registrada exitosamente")
        window.setTimeout(function () {
            location.reload()
        }, 3500)
    }else if(json.code == 500){
...
const getTally = async(e) => {

    e.preventDefault();

    let sucursal = document.getElementById("tallySucursal")
    let sucursalHelp = document.getElementById("sucursalHelp")
    ...
    let response = await fetch('/getEntriesEgress', {
        method: 'POST',
        mode: 'cors',
        cache: 'no-cache',
        credentials: 'same-origin',
    ...
    let json = await response.json()
    document.body.style.cursor = 'auto'
    let table = document.getElementById('entriesEgress')
    let alert = document.getElementById('alert')

    if(json.code == 201){

        let html = `<h3 class="text-center mb-5 mt-5">Ingresos</h3>`
        if(json.data.entries.length > 0){
            ...
            <table class="table mt-3">
                ...
                <tbody>
            `
            for(let i = 0; i < json.data.entries.length; i++ ){
                html += `
                    <tr>
                        <td>${json.data.entries[i].can_ing}</td>
                        <td>${json.data.entries[i].fec_ing}</td>
                    </tr>
                `
            }
            html += `
            ...
            html += `<h5 class="text-center mt-4">No hay coincidencias</h5>`
        }

        html += `<h3 class="text-center mb-5 mt-5">Egresos</h3>`

        if(json.data.egress.length > 0){
            html += `
            <table class="table mt-3">
                <thead>
                    <th>Cantidad</th>
                ...
                <tbody>
            `
            for(let i = 0; i < json.data.egress.length; i++ ){
                html += `
                    <tr>
                        <td>${json.data.egress[i].can_egr}</td>
                        <td>${json.data.egress[i].fec_egr}</td>
                    </tr>
                `
            }
            html += `
...
const getPaymentsSucursal = async (e) => {

    e.preventDefault()

    let sucursal = document.getElementById("paymentsSucursal")
    let sucursalHelp = document.getElementById("paymentsSucursalHelp")
    ...
    let response = await fetch('/getPaymentsSucursal', {
        method: 'POST',
        mode: 'cors',
        cache: 'no-cache',
        credentials: 'same-origin',
    ...
    if(json.code == 201){
        ...
        let html = `<h3 class="text-center mb-5 mt-5">Pagos en el día</h3>`
        if(json.data.length > 0){
            ...
            for(let i = 0; i < json.data.length; i++ ){
                html += `
                    <tr>
                        <td>${decrypt(json.data[i].name)}</td>
                        <td>${json.data[i].fec_pag}</td>
                        <td>${json.data[i].can_pag}</td>
                    </tr>
                `
            }
            html += `
